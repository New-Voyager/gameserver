...
func (s *SeatsInPots) add(seatNo uint32, amount float64) {
	found := false
	for i := range s.Seats {
		if seatNo == s.Seats[i] {
			// already in the pot
...
func (h *HandState) lowestBet(seatBets []float64) float64 {
	lowestBet := float64(-1.0)
...
func (h *HandState) addChipsToPot(seatBets []float64, handEnded bool) {
	...
	for seatNo, bet := range seatBets {
		...
		_, exists := h.PotContribution[uint32(seatNo)]
		if !exists {
		...
		if bet < lowestBet {
			...
			h.PotContribution[uint32(seatNo)] += bet
			h.PlayersActed[seatNo].BetAmount += bet
			seatBets[seatNo] = 0
		} else {
			currentPot.add(uint32(seatNo), lowestBet)
			h.PotContribution[uint32(seatNo)] += lowestBet
